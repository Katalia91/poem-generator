...
function generatePoem(event) {
  ...
  let context = `You are an AI assistant and poet. Please generate a poem about the chosen topic. It should have not more than 6 lines by default, but these max 6 lines should be a finished poen, it shouldn't just stop with a comma. It should be in the following format, so that each line is a separate <p></p> like this: <p>First line</p></p>Second line</p> and so on. After finished poem use <br /><br /> and sign with <em> -Your personal poet</em>🖋️`;
  let prompt = `Generate a poem about ${userInput}`;
  let apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  poemContent.classList.toggle("hidden");
  poemContent.innerHTML = `<span class="blink">⏳ Generating a poem about: <strong>${userInput}</strong>...</span>`;
